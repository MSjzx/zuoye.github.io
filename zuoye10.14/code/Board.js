...
Board.prototype.init = function () {
    ...
    for(var row = 0; row < this.rows; row++) {
        ...
        for (var col = 0; col < this.cols; col++) {
            if (row === 0 && col === 0) {
                // 左上角
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.leftTop));
            } else if (row === 0 && col === this.cols - 1) {
                // 右上角
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.rightTop));
            } else if (row === this.rows - 1 && col === 0) {
                // 左下角
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.leftBottom));
            } else if (row === this.rows - 1 && col === this.cols - 1) {
                // 右下角
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.rightBttom));
            } else if (row !== 0 && row != this.rows - 1 && col === 0) {
                // 左边
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.left));
            } else if (row !== 0 && row != this.rows - 1 && col === this.cols - 1) {
                // 右边
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.right));
            } else if (row === 0 && col !== 0 && col !== this.cols - 1) {
                // 上边
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.top));
            } else if (row === this.rows - 1 && col !== 0 && col !== this.cols - 1) {
                // 下边
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.bottom));
            } else {
                temp.push(new Cell(this.cellSize, col, row, Cell.Bgcs.center));
            }
        }
